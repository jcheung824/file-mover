...
function calculateTrends(history) {
  ...
  const trends = {
    ...
    fileDiscoveryTime: calculatePercentageChange(previous.metrics.fileDiscovery.time, latest.metrics.fileDiscovery.time),
...
function displayTrends(trends) {
  ...
  console.log('─────────────────────────────────────────────────────────────');
  console.log(`Total Time per File        │ ${trendColor(trends.totalTimePerFile)}${trends.totalTimePerFile.toFixed(1).padStart(8)}%${trendEmoji(trends.totalTimePerFile)}\x1b[0m │ ${getTrendStatus(trends.totalTimePerFile)}`);
  console.log(`Average Analysis Time      │ ${trendColor(trends.avgAnalysisTime)}${trends.avgAnalysisTime.toFixed(1).padStart(8)}%${trendEmoji(trends.avgAnalysisTime)}\x1b[0m │ ${getTrendStatus(trends.avgAnalysisTime)}`);
  console.log(`Average Move Time          │ ${trendColor(trends.avgMoveTime)}${trends.avgMoveTime.toFixed(1).padStart(8)}%${trendEmoji(trends.avgMoveTime)}\x1b[0m │ ${getTrendStatus(trends.avgMoveTime)}`);
  console.log(`Average Update Time        │ ${trendColor(trends.avgUpdateTime)}${trends.avgUpdateTime.toFixed(1).padStart(8)}%${trendEmoji(trends.avgUpdateTime)}\x1b[0m │ ${getTrendStatus(trends.avgUpdateTime)}`);
  console.log(`File Discovery Time        │ ${trendColor(trends.fileDiscoveryTime)}${trends.fileDiscoveryTime.toFixed(1).padStart(8)}%${trendEmoji(trends.fileDiscoveryTime)}\x1b[0m │ ${getTrendStatus(trends.fileDiscoveryTime)}`);
  console.log(`Validation Time            │ ${trendColor(trends.validationTime)}${trends.validationTime.toFixed(1).padStart(8)}%${trendEmoji(trends.validationTime)}\x1b[0m │ ${getTrendStatus(trends.validationTime)}`);
}
...
function generateComparisonTable(history) {
  ...
  console.log('RUN ID                    │ TIME/FILE │ FILES │ MOVES │ AVG ANALYSIS │ FILE DISC │ VALIDATION │ FILE OPS');
  console.log('────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────');

  recentRuns.forEach(run => {
    const timePerFile = run.summary.totalFiles > 0 ? (run.metrics.totalTime / run.summary.totalFiles).toFixed(2) : '0.00';
    
    console.log(`${run.runId.padEnd(26)} │ ${timePerFile.padStart(9)}ms │ ${run.summary.totalFiles.toString().padStart(5)} │ ${run.summary.totalMoves.toString().padStart(5)} │ ${run.summary.avgAnalysisTime.toFixed(2).padStart(11)}ms │ ${run.metrics.fileDiscovery.time.toFixed(2).padStart(8)}ms │ ${run.metrics.validation.time.toFixed(2).padStart(9)}ms │ ${run.metrics.fileOperations.time.toFixed(2).padStart(8)}ms`);
  });
}
...
function generateImprovementAnalysis(history) {
  ...
  const improvements = {
    ...
    fileDiscoveryTime: calculatePercentageChange(firstRun.metrics.fileDiscovery.time, latestRun.metrics.fileDiscovery.time),
  ...
  console.log(`  Total Time per File: ${improvements.totalTimePerFile > 0 ? '+' : ''}${improvements.totalTimePerFile.toFixed(1)}%`);
  console.log(`  Average Analysis Time: ${improvements.avgAnalysisTime > 0 ? '+' : ''}${improvements.avgAnalysisTime.toFixed(1)}%`);
  console.log(`  File Discovery Time: ${improvements.fileDiscoveryTime > 0 ? '+' : ''}${improvements.fileDiscoveryTime.toFixed(1)}%`);
  console.log(`  Validation Time: ${improvements.validationTime > 0 ? '+' : ''}${improvements.validationTime.toFixed(1)}%`);
  ...
  console.log(`Worst Performance Run: ${worstRun.runId} (${worstRun.summary.avgAnalysisTime.toFixed(2)}ms avg analysis)`);
}
...
function generateRecommendations(history) {
  ...
  const totalAnalysisTime = latestRun.metrics.importAnalysis.fileReadTime + latestRun.metrics.importAnalysis.astParseTime + latestRun.metrics.importAnalysis.importMatchingTime;
...
function generateHTMLContent(history) {
  const chartData = history.map(run => ({
    ...
    precomputeTime: run.metrics.importAnalysis.time - (run.metrics.importAnalysis.fileReadTime + run.metrics.importAnalysis.astParseTime + run.metrics.importAnalysis.importMatchingTime),
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: data.map(d => d.totalTimePerFile),
                    borderColor: 'rgb(75, 192, 192)',
                    backgroundColor: 'rgba(75, 192, 192, 0.2)',
                    yAxisID: 'y'
                }, {
                    label: 'Avg Analysis Time (ms)',
                    data: data.map(d => d.avgAnalysisTime),
                    borderColor: 'rgb(255, 99, 132)',
                    backgroundColor: 'rgba(255, 99, 132, 0.2)',
                    yAxisID: 'y'
                }, {
                    label: 'File Discovery (ms)',
                    data: data.map(d => d.fileDiscoveryTime),
                    borderColor: 'rgb(54, 162, 235)',
                    backgroundColor: 'rgba(54, 162, 235, 0.2)',
                    yAxisID: 'y'
                }, {
                    label: 'Validation (ms)',
                    data: data.map(d => d.validationTime),
