...
import { normalizePath, removeExtension } from "./pathUtils.js";
...
export async function parseFileContent(filePath: string, config: Config): Promise<{ ast: any; content: string } | null> {
  try {
    ...
    if (config.verbose) {
      console.warn(
        `⚠️  Could not parse file ${filePath}: ${e instanceof Error ? e.message : String(e)}`
      );
    }
    return null;
  }
