    ...
    for (let i = 0; i < normalizedMoves.length; i++) {
        ...
        try {
            ...
            for (const { file, imports } of importAnalysis) {
                const updated = await updateImportsInFile(file, imports, fromPath, toPath);
                if (updated)
                    updatedFiles++;
            }
            if (updatedFiles > 0) {
 ...
 */
function generateTargetImportPaths(targetPath) {
 ...
 */
function matchesTarget(importPath, targetImportPaths, currentFile) {
    ...
    if (importPath.startsWith("./") || importPath.startsWith("../")) {
        ...
        return targetImportPaths.some((targetPath) => normalizePath(resolvedPath) === targetPath ||
...
function getMsImportPath(filePath) {
 ...
 */
function generateNewMsImportPath(newPath) {
 ...
 * Update imports inside the moved file itself
 */
async function updateImportsInMovedFile(oldPath, newPath) {
    try {
        ...
        console.error(`❌ Error updating imports in moved file ${newPath}:`, error instanceof Error ? error.message : String(error));
    }
}
async function updateImportsInFile(filePath, imports, oldPath, newPath) {
    try {
